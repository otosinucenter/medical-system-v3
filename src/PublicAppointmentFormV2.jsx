...
export default function PublicAppointmentFormV2() {
    ...
    useEffect(() => {
        ...
        useEffect(() => {
            ...
            // Reset time if not valid for new date (unless it's 'other')
            if (formData.time && !slots.includes(formData.time) && formData.time !== 'other') {
                // Check if it's blocked by smart logic later, for now just keep if valid format
            }

            // Fetch booked slots and calculate intervals
            const fetchBookedSlots = async () => {
                ...
                if (data) {
                    const intervals = data.map(apt => {
                        const d = new Date(apt.appointment_date);
                        const start = d.toTimeString().slice(0, 5); // "HH:mm"

                        // Calculate end time (Start + 20 mins)
                        const endD = new Date(d.getTime() + 20 * 60000);
                        const end = endD.toTimeString().slice(0, 5);

                        return { start, end };
                    });
                    setBookedIntervals(intervals);
                    setBookedSlots(data.map(a => new Date(a.appointment_date).toTimeString().slice(0, 5))); // Keep for legacy check if needed
                }
            };
        ...
        const isSlotBlocked = (slotTime) => {
            // Slot interval: slotTime -> slotTime + 20m
            ...
            return bookedIntervals.some(booked => {
                ...
                // Using string comparison works for same-day times "HH:mm"
                return (startStr < booked.end) && (endStr > booked.start);
            });
        };
                    ...
                    <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-2">Fecha Propuesta</label>
                                    <input
                                        type="date"
                                        required
                                        className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm"
                                        value={formData.date}
                                        onChange={e => setFormData({ ...formData, date: e.target.value, time: '' })}
                                    />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-2">Selecciona tu hora de llegada</label>
                                    {availableSlots.length > 0 ? (
                                        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
                                            <div className="grid grid-cols-3 gap-2">
                                                {availableSlots.filter(slot => !isSlotBlocked(slot)).map(slot => (
                                                    <button
                                                        key={slot}
                                                        type="button"
                                                        onClick={() => setFormData({ ...formData, time: slot })}
                                                        className={`
                                                        relative py-2 px-1 rounded-lg border text-center transition-all duration-200
                                                        ${formData.time === slot
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-2 px-4 rounded-lg border text-sm font-medium transition-all flex items-center justify-center gap-2 ${formData.time === 'other'
                                                    ? 'bg-indigo-50 text-indigo-700 border-indigo-200'
                                                    : 'bg-transparent text-slate-500 border-transparent hover:bg-slate-50 hover:text-slate-700'
                                                    }`}
                                            >
                                                <Clock className="w-4 h-4" />
                                                {formData.time === 'other' ? 'Horario personalizado seleccionado' : 'Proponer otro horario'}
                                            </button>
                                        </div>
                                    ) : (
                                        <div className="text-sm text-slate-500 p-6 bg-slate-50 rounded-xl border border-slate-200 text-center">
                                            <Clock className="w-8 h-8 mx-auto text-slate-300 mb-2" />
                                            <p className="mb-4">{formData.date ? "No hay horarios disponibles para esta fecha." : "Selecciona una fecha en el calendario."}</p>
                                            {formData.date && (
                                                <button
                                                    type="button"
                                                    onClick={() => setFormData({ ...formData, time: 'other' })}
    ...
    export default PublicAppointmentFormV2;
